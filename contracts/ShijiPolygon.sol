...
contract ShijiPolygon is ERC1155, ERC1155URIStorage, IDapp, FunctionsClient, ConfirmedOwner {
  ...
  event ShijiClaimed(bytes32 indexed requestId, uint256 shijiId, address claimer, uint256 timestamp);
  ...
  function createShiji(
    string memory sourceCode,
    string memory metadataUrl
  ) external onlyOwner returns (uint256 _shijiId) {
  ...
  function claimShiji(
    ...
    bytes32 requestId = _executeRequest(shijiToSourceCode[shijiId], secrets, args, subscriptionId, gasLimit);
    ...
    if (recipient != address(0)) {
      if (response.length > 0) {
        ...
        if (condition > 0) {
          ...
          emit ShijiClaimed(requestId, requestRegistry[requestId].shijiId, recipient, block.timestamp);
        } else {
